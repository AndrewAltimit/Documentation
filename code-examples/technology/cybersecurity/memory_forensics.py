...
class MemoryForensics:
    ...
    def parse_eprocess(self, offset):
        ...
        try:
            ...
            eprocess = {
                'offset': offset,
                'pid': struct.unpack('<I', self.memory[offset+0x2e0:offset+0x2e4])[0],
                'ppid': struct.unpack('<I', self.memory[offset+0x2e8:offset+0x2ec])[0],
                'image_name': self.memory[offset+0x450:offset+0x460].decode('utf-8', errors='ignore').strip('\x00'),
                'create_time': struct.unpack('<Q', self.memory[offset+0x2f0:offset+0x2f8])[0],
                'exit_time': struct.unpack('<Q', self.memory[offset+0x2f8:offset+0x300])[0],
            }
    ...
    def is_aes_key_schedule(self, data):
        ...
        for i in range(len(data) - 1):
            if data[i] ^ data[i+1] in [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80]:
                sbox_pattern += 1
        ...
        try:
            if signature == b'regf':
                ...
                hive = {
                    'offset': offset,
                    'signature': signature,
                    'sequence1': struct.unpack('<I', self.memory[offset+4:offset+8])[0],
                    'sequence2': struct.unpack('<I', self.memory[offset+8:offset+12])[0],
                    'timestamp': struct.unpack('<Q', self.memory[offset+12:offset+20])[0],
                    'major_version': struct.unpack('<I', self.memory[offset+20:offset+24])[0],
                    'minor_version': struct.unpack('<I', self.memory[offset+24:offset+28])[0],
                }
                
                # Extract hive name
                name_offset = offset + 48
                name_length = 64
                hive['name'] = self.memory[name_offset:name_offset+name_length].decode('utf-16le', errors='ignore').strip('\x00')
    ...
    def parse_handle(self, offset):
        ...
        try:
            handle = {
                'offset': offset,
                'object_pointer': struct.unpack('<Q', self.memory[offset:offset+8])[0],
                'granted_access': struct.unpack('<I', self.memory[offset+8:offset+12])[0],
                'handle_attributes': struct.unpack('<I', self.memory[offset+12:offset+16])[0]
            }
    ...
    def parse_single_mft_entry(self, offset):
        ...
        try:
            entry = {
                'offset': offset,
                'sequence': struct.unpack('<H', self.memory[offset+16:offset+18])[0],
                'link_count': struct.unpack('<H', self.memory[offset+18:offset+20])[0],
            ...
            while current_offset < offset + 1024:
                attr_type = struct.unpack('<I', self.memory[current_offset:current_offset+4])[0]
                ...
                if attr_type == 0x10:  # $STANDARD_INFORMATION
                    ...
                    entry['timestamps'] = [
                        {
                            'timestamp': struct.unpack('<Q', self.memory[si_offset:si_offset+8])[0],
                            'type': 'File Created',
                            'details': f"MFT Entry: {entry['sequence']}"
                        },
                        {
                            'timestamp': struct.unpack('<Q', self.memory[si_offset+8:si_offset+16])[0],
                            'type': 'File Modified',
                            'details': f"MFT Entry: {entry['sequence']}"
                        }
                    ]
                ...
                attr_length = struct.unpack('<I', self.memory[current_offset+4:current_offset+8])[0]
