...
class AdversarialML:
    ...
    def pgd_attack(self, model, image, label, epsilon=0.3, 
    ...
    def carlini_wagner_attack(self, model, image, num_classes, 
                              target_class=None, c=1.0, kappa=0, 
            ...
            for step in range(max_iter):
                with tf.GradientTape() as tape:
                    ...
                    if target_class is not None:
                        # Targeted attack
                        real = tf.reduce_max(logits - tf.one_hot(target_class, num_classes) * 1e10)
                        target = logits[0, target_class]
                        loss_f = tf.maximum(real - target + kappa, 0)
                    else:
                        # Untargeted attack
                        real = logits[0, tf.argmax(logits[0])]
                        second = tf.reduce_max(logits - tf.one_hot(tf.argmax(logits[0]), num_classes) * 1e10)
    ...
    def universal_perturbation(self, model, images, num_classes, 
                              delta=0.8, max_iter=20, xi=10, p=np.inf):
        """
        ...
        for iteration in range(max_iter):
            ...
            for image in images:
                ...
                if orig_pred != new_pred:
                    ...
                    _, delta_v = self.deepfool_attack(
                        model, perturbed, num_classes
                    )
...
class PrivacyAttacks:
    ...
    def membership_inference(self, target_model, target_data, target_labels, 
                            shadow_models, shadow_data):
        """
    ...
    def model_inversion(self, model, target_class, num_features, 
                        regularization=1.0, learning_rate=0.1, iterations=1000):
        """
