    ...
    def build_precedence_graph(self) -> Dict[int, Set[int]]:
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if (
                    op1.txn_id != op2.txn_id
                    and op1.item == op2.item
                    and op1.item is not None
                ):
...
class MVCCScheduler:
    ...
    def read(self, txn_id: int, item: str) -> Any:
        ...
        with self.lock:
            ...
            valid_versions = [
                v for v in self.versions[item] if v.write_timestamp <= txn_start
            ]
            ...
            for item in txn_info["read_set"]:
                if item in self.versions:
                    for version in self.versions[item]:
                        ...
                        if (
                            version.write_timestamp > txn_info["start_time"]
                            and version.write_timestamp < txn_id
                        ):
    ...
    def garbage_collect(self, item: str):
        ...
        if self.active_transactions:
            active_txns = [
                t for t in self.active_transactions.values() if t["state"] == "active"
            ]
            if active_txns:
        ...
        if self.versions[item]:
            self.versions[item] = sorted(
                self.versions[item], key=lambda v: v.write_timestamp
            )[
                -5:
            ]  # Keep last 5 versions max
...
class LockManager:
    ...
    def __init__(self):
        self.locks = defaultdict(
            lambda: {"mode": None, "holders": set(), "waiters": deque()}
        )
    ...
    def _already_holds_lock(self, txn_id: int, item: str, mode: LockMode) -> bool:
        ...
        for held_item, held_mode in self.txn_locks[txn_id]:
            if held_item == item:
                ...
                if mode == LockMode.SHARED and held_mode in [
                    LockMode.SHARED,
                    LockMode.EXCLUSIVE,
                ]:
                    return True
...
class TimestampOrderingScheduler:
    ...
    def begin_transaction(self) -> int:
        ...
        with self.lock:
            ...
            self.timestamp_counter += 1
            self.transaction_timestamps[ts] = {"start": ts, "state": "active"}
            return ts
