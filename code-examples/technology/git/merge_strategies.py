...
class MergeStrategy(ABC):
    ...
    def _is_ancestor(self, potential_ancestor: str, 
                    descendant: str) -> bool:
...
class RecursiveMergeStrategy(MergeStrategy):
    ...
    def _three_way_merge(self, base: Commit, ours: Commit, 
                        theirs: Commit) -> MergeResult:
        ...
        merged_tree = self._merge_trees(base_tree, our_tree, 
                                      their_tree, result)
    ...
    def _merge_trees(self, base: Tree, ours: Tree, 
    ...
    def _merge_entry(self, path: str, base: Optional[TreeEntry],
                    ours: Optional[TreeEntry], 
                    theirs: Optional[TreeEntry],
                    result: MergeResult) -> Optional[TreeEntry]:
    ...
    def _entries_equal(self, e1: Optional[TreeEntry], 
                      e2: Optional[TreeEntry]) -> bool:
    ...
    def _merge_file_content(self, base: bytes, ours: bytes, 
                           theirs: bytes) -> Optional[bytes]:
    ...
    def _merge_unrelated(self, ours: Commit, 
                        theirs: Commit) -> MergeResult:
...
class OctopusMergeStrategy(MergeStrategy):
    ...
    def merge_multiple(self, head: Commit, 
        ...
        if not common_base:
            return MergeResult(
                success=False,
                message="No common base found for octopus merge"
            )
    ...
    def _find_common_base_all(self, commits: List[Commit]) -> Optional[Commit]:
        ...
        return Commit(sha=list(common)[0], tree=Tree(entries={}), 
                     parents=[], message="Common base")
    
    def _multi_way_merge(self, base: Commit, 
                        branches: List[Commit]) -> Optional[Tree]:
    ...
    def _compute_changes(self, base: Tree, 
                        branch: Tree) -> Dict[str, TreeEntry]:
...
class SubtreeMergeStrategy(MergeStrategy):
    ...
    def merge(self, base: Commit, ours: Commit, 
        ...
        their_commit = Commit(theirs.sha, their_subtree, theirs.parents, 
                            theirs.message)
        
        result = recursive._three_way_merge(base_commit, our_commit, 
                                          their_commit)
