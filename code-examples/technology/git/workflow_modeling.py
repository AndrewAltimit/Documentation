...
class GitWorkflow(ABC):
    ...
    def validate_transition(self, from_branch: str, 
                          to_branch: str) -> Tuple[bool, Optional[str]]:
        ...
        for rule in self.transition_rules:
            if rule.from_state == from_state and rule.to_state == to_state:
                if rule.condition:
                    ...
                    if not self._evaluate_condition(rule.condition, from_branch, to_branch):
    ...
    def suggest_next_actions(self, current_branch: str) -> List[Dict[str, str]]:
        ...
        for state in reachable:
            ...
            actions.append({
                ...
                'example_branch': example_branches[0] if example_branches else f"{state.value}/*",
    ...
    def _evaluate_condition(self, condition: str, from_branch: str, to_branch: str) -> bool:
...
class MonorepoWorkflow(GitWorkflow):
    ...
    def validate_changes(self, branch: str, changed_files: List[str]) -> List[WorkflowViolation]:
...
class WorkflowAnalyzer:
    ...
    def calculate_metrics(self, branch_history: List[Tuple[str, str, str]]) -> Dict[str, any]:
...
def demo_workflows():
    ...
    # Validate transition
    valid, reason = gitflow.validate_transition("feature/user-authentication", "develop")
