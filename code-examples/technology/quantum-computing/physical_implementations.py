...
class TrappedIonQubit:
    ...
    def addressing_errors(self, target_ion: int, beam_width: float, 
                         ion_spacing: float) -> Dict[int, float]:
    ...
    def braiding_operator(self, exchange_type: str = 'clockwise') -> np.ndarray:
        ...
        if exchange_type == 'clockwise':
            ...
            return np.array([
                ...
                [0, 1/np.sqrt(2), -1/np.sqrt(2), 0],
                [0, 0, 0, 1]
            ], dtype=complex) * np.exp(1j * np.pi/4)
        else:
...
class NeutralAtomQubit:
    ...
    def rydberg_gate(self, control: int, target: int, 
