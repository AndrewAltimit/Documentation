...
class SimpleHarmonicMotion(Scene):
    ...
    def construct(self):
        ...
        # Wall
        wall = Line(
            start=spring_start + UP,
            end=spring_start + DOWN,
            stroke_width=8
        )
        ...
        sine_curve = axes.plot(
            lambda t: amplitude * np.sin(omega * t),
            color=RED,
            x_range=[0, 0.1]
        )
        ...
        def update_curve(mob):
            t = time_tracker.get_value()
            new_curve = axes.plot(
                lambda x: amplitude * np.sin(omega * x),
                color=RED,
                x_range=[0, t]
            )
...
class PendulumPhaseSpace(Scene):
    ...
    def construct(self):
        ...
        # Add explanation
        explanation = Text(
            "Phase space shows all possible states of the system",
            font_size=20
        ).to_edge(DOWN)
