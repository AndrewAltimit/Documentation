...
def create_simple_harmonic_motion():
    ...
    # Mass
    mass_size = 0.6
    mass = FancyBboxPatch((-0.3, -0.3), mass_size, mass_size, 
    ...
    # Mass label
    mass_text = ax1.text(0, 0, 'm', ha='center', va='center', 
...
def create_wave_interference():
    ...
    ax.text(source1_pos[0], source1_pos[1]-0.5, 'S₁', ha='center', 
            color='red', fontsize=12, weight='bold')
    ax.text(source2_pos[0], source2_pos[1]-0.5, 'S₂', ha='center', 
    ...
    im = ax.imshow(np.zeros_like(X), extent=[-5, 5, -5, 5], 
    ...
    def animate(frame):
        ...
        for i in range(max_circles):
            ...
            if alpha > 0.1:
                circle1 = Circle(source1_pos, radius, fill=False, 
                               edgecolor='red', alpha=alpha * 0.3, linewidth=1)
                circle2 = Circle(source2_pos, radius, fill=False, 
                               edgecolor='blue', alpha=alpha * 0.3, linewidth=1)
    ...
    # Add legend
    ax.text(0, 4.5, 'Constructive Interference (Red)', ha='center', 
            color='white', fontsize=10, bbox=dict(boxstyle="round,pad=0.3", 
                                                 facecolor='darkred', alpha=0.5))
    ax.text(0, -4.5, 'Destructive Interference (Blue)', ha='center', 
            color='white', fontsize=10, bbox=dict(boxstyle="round,pad=0.3", 
...
def create_pendulum_phase_space():
    ...
    contours = ax2.contour(Theta, Omega, E, levels=10, colors='gray', 
    ...
    ax2.text(0.02, 0.98, 'Energy contours', transform=ax2.transAxes, 
