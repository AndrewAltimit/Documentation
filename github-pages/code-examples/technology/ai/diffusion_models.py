...
class ScoreBasedDiffusion:
    ...
    def __init__(self, score_model: nn.Module, sigma_min: float = 0.01, 
    ...
    def sde_diffusion(self, t: torch.Tensor) -> torch.Tensor:
        ...
        return torch.sqrt(2 * torch.log(self.sigma_max / self.sigma_min) * sigma ** 2 / 
                         (self.sigma_max / self.sigma_min) ** (2 * t))
    
    def reverse_sde(self, x: torch.Tensor, t: torch.Tensor, 
    ...
    @torch.no_grad()
    def sample(self, shape: Tuple[int, ...], device: str = 'cuda', 
...
class DDPM:
    ...
    def p_mean_variance(self, x_t: torch.Tensor, t: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        mean = sqrt_recip_alphas_t * (x_t - beta_t / sqrt_one_minus_alphas_cumprod_t * epsilon_theta)
    ...
    @torch.no_grad()
    def ddim_sample(self, shape: Tuple[int, ...], device: str = 'cuda',
        ...
        for i in reversed(range(ddim_timesteps)):
            ...
            alpha_t_prev = self.alphas_cumprod_prev[t][:, None, None, None] if i > 0 else torch.ones_like(alpha_t)
            
            # Predict x_0
            x_0_pred = (x - torch.sqrt(1 - alpha_t) * epsilon_theta) / torch.sqrt(alpha_t)
            
            # Direction pointing to x_t
            sigma_t = eta * torch.sqrt((1 - alpha_t_prev) / (1 - alpha_t) * (1 - alpha_t / alpha_t_prev))
            
            # Compute x_{t-1}
            mean_pred = (torch.sqrt(alpha_t_prev) * x_0_pred + 
...
class LatentDiffusionModel(nn.Module):
    ...
    def __init__(self, vae: nn.Module, diffusion_model: nn.Module,
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor,
                conditioning: Optional[Dict[str, torch.Tensor]] = None) -> torch.Tensor:
    ...
    def sample(self, shape: Tuple[int, ...], 
        ...
        latent_shape = (shape[0], 4, shape[2]//8, shape[3]//8)  # Assuming 8x downsampling
...
class GuidedDiffusion:
    ...
    def classifier_guided_sample(self, shape: Tuple[int, ...], 
                                class_label: int, guidance_scale: float = 1.0,
                                device: str = 'cuda') -> torch.Tensor:
    ...
    def classifier_free_guidance(self, shape: Tuple[int, ...],
    ...
    def _get_mean_from_epsilon(self, x_t: torch.Tensor, t: torch.Tensor,
                              epsilon: torch.Tensor) -> torch.Tensor:
        """Compute mean from noise prediction"""
        beta_t = self.diffusion_model.betas[t][:, None, None, None]
        sqrt_one_minus_alphas_cumprod_t = self.diffusion_model.sqrt_one_minus_alphas_cumprod[t][:, None, None, None]
        sqrt_recip_alphas_t = self.diffusion_model.sqrt_recip_alphas[t][:, None, None, None]
        
        return sqrt_recip_alphas_t * (x_t - beta_t / sqrt_one_minus_alphas_cumprod_t * epsilon)