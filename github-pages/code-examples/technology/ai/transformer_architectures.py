...
class VisionTransformer(nn.Module):
    ...
    def get_2d_sincos_pos_embed(
        embed_dim: int, grid_size: int, cls_token: bool = True
    ) -> np.ndarray:
        ...
        pos_embed[:, 0 : embed_dim // 2] = np.sin(
            grid[0].reshape(-1, 1) * np.arange(embed_dim // 2) * 2 * np.pi / grid_size
        )
        pos_embed[:, embed_dim // 2 :] = np.cos(
            grid[1].reshape(-1, 1) * np.arange(embed_dim // 2) * 2 * np.pi / grid_size
        )
...
class CLIP(nn.Module):
    ...
    def compute_retrieval_metrics(
        ...
        i2t_correct = i2t_indices == torch.arange(
            len(similarity), device=similarity.device
        ).unsqueeze(1)

        # Text-to-image retrieval
        _, t2i_indices = similarity.t().topk(10, dim=1)
        t2i_correct = t2i_indices == torch.arange(
            len(similarity), device=similarity.device
        ).unsqueeze(1)
...
class BERTModel(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                TransformerBlock(
                    hidden_size, num_heads, mlp_ratio=intermediate_size / hidden_size
                )
    ...
    def forward(
        ...
        return {"last_hidden_state": hidden_states, "pooler_output": pooled_output}
