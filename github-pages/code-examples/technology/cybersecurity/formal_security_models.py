...
class ShamirSecretSharing:
    ...
    def share_secret(self, secret):
        ...
        coefficients = [secret] + [
            random.randrange(0, self.p) for _ in range(self.t - 1)
        ]
        ...
        for x in range(1, self.n + 1):
            y = (
                sum(coef * pow(x, i, self.p) for i, coef in enumerate(coefficients))
                % self.p
            )
            shares.append((x, y))
...
class ObliviousTransfer:
    ...
    def sender_setup(self, m0, m1):
        ...
        self.sender_state = {"a": a, "A": A, "m0": m0, "m1": m1}
    ...
    def receiver_choose(self, b, A):
        ...
        self.receiver_state = {"b": b, "x": x, "X": X}
