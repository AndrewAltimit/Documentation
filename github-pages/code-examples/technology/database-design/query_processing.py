...
class CostModel:
    ...
    def estimate_scan_cost(self, table_stats: TableStats, 
        ...
        seq_cost = (self.seq_page_cost * pages + 
                   self.cpu_tuple_cost * tuples * selectivity)
        
        return seq_cost
    
    def estimate_index_scan_cost(self, table_stats: TableStats, 
        ...
        # Total cost
        cost = (self.random_page_cost * index_pages +
                self.random_page_cost * data_pages +
                self.cpu_tuple_cost * table_stats.num_rows * selectivity)
        
        return cost
    
    def estimate_join_cost(self, method: str, left_size: int, 
                          right_size: int) -> float:
        ...
        if method == 'nested_loop':
            ...
            # Sort both + merge
            sort_cost = (left_size * np.log2(left_size) + 
                        right_size * np.log2(right_size)) * self.cpu_operator_cost
    ...
    def estimate_aggregate_cost(self, num_tuples: int, 
                               group_by_columns: int) -> float:
...
class QueryOptimizer:
    ...
    def _rebuild_plan_with_order(self, original: QueryPlan, 
                                join_order: List[str]) -> QueryPlan:
...
def demonstrate_query_optimization():
    ...
    stats = {
        'users': TableStats(
            ...
            avg_row_size=200,
            columns={
                'id': ColumnStats('id', distinct_values=10000, min_value=1, 
                                 max_value=10000, null_count=0),
                'age': ColumnStats('age', distinct_values=80, min_value=18, 
                                  max_value=100, null_count=0)
            }
        ),
        'orders': TableStats(
            name='orders',
            ...
            avg_row_size=100,
            columns={
                'user_id': ColumnStats('user_id', distinct_values=8000, 
                                      min_value=1, max_value=10000, null_count=0)
            }
        ),
        'products': TableStats(
            name='products',
    ...
    root = QueryPlan('project', [join2], params={'columns': ['user_id', 'product_name']})
