...
class RelationalAlgebra:
    ...
    def difference(R: pd.DataFrame, S: pd.DataFrame) -> pd.DataFrame:
        ...
        return (
            R.merge(S, how="left", indicator=True)
    ...
    def aggregate(
        ...
        group_by: List[str],
        agg_funcs: Dict[str, Tuple[str, Callable]],
    ) -> pd.DataFrame:
    ...
    def execute(self, database: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        if self.operation == "relation":
            ...
            if "condition" in self.params:
                return RelationalAlgebra.theta_join(
                    left_result, right_result, self.params["condition"]
                )
            else:
            ...
            child_result = self.children[0].execute(database)
            return RelationalAlgebra.aggregate(
                child_result, self.params.get("group_by", []), self.params["agg_funcs"]
            )
...
def demonstrate_relational_algebra():
    ...
    cs_students = RelationalAlgebra.selection(
        students, lambda row: row["major"] == "CS"
    )
...
def optimize_query_example():
    ...
    select_tree = QueryTree(
        "selection",
        children=[join_tree],
        params={"predicate": lambda r: r["major"] == "CS"},
    )
    project_tree = QueryTree(
        "projection", children=[select_tree], params={"attributes": ["name"]}
    )
