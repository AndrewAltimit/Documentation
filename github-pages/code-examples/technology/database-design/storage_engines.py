...
class Page:
    ...
    def insert_record(self, record: bytes) -> Optional[int]:
        ...
        available_space = self.PAGE_SIZE - self.free_space_offset - (self.slot_count * slot_size)
        
        if available_space < needed_space:
            return None  # Not enough space
        
        # Write record at free space offset
        self.data[self.free_space_offset:self.free_space_offset + record_size] = record
        ...
        struct.pack_into('II', self.data, slot_offset, self.slots[slot_num][0], self.slots[slot_num][1])
...
class BPlusTree:
    ...
    def _redistribute_from_left(self, node: BPlusTreeNode, left_sibling: BPlusTreeNode, 
                               parent: BPlusTreeNode, parent_idx: int):
    ...
    def _redistribute_from_right(self, node: BPlusTreeNode, right_sibling: BPlusTreeNode,
                                parent: BPlusTreeNode, parent_idx: int):
    ...
    def _merge_nodes(self, left: BPlusTreeNode, right: BPlusTreeNode,
                    parent: BPlusTreeNode, parent_idx: int):
        """Merge two nodes"""
...
class LSMTree:
        ...
        for level in reversed(range(len(self.levels))):
            for sstable in self.levels[level]:
                for entry in sstable.range_scan(start_key, end_key):
                    if entry.key not in results or results[entry.key][1] < entry.timestamp:
    ...
    def _compact_level(self, level: int):
        ...
        for i, sstable in enumerate(new_sstables):
            self._save_sstable(sstable, level + 1, 
                             len(self.levels[level + 1]) - len(new_sstables) + i)
