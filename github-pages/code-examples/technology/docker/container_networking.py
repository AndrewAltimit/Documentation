...
class CNIPlugin:
    ...
    def add_network(self, container_id: str, netns: str, 
                   ifname: str, config: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        result["interfaces"].append({
            "name": veth_host,
            "mac": self._get_mac_address(None, veth_host)
        })
    ...
    def delete_network(self, container_id: str, netns: str, 
                      ifname: str, config: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def check_network(self, container_id: str, netns: str, 
                     ifname: str, config: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _configure_container_interface(self, netns: str, ifname: str, 
                                     config: Dict[str, Any]):
...
class DockerBridge:
    ...
    def create_bridge(self):
        ...
        # Create bridge
        subprocess.run([
            "ip", "link", "add", self.bridge_name, 
            "type", "bridge"
        ], check=True)
        
        # Set MTU
        subprocess.run([
            "ip", "link", "set", self.bridge_name, 
            "mtu", str(self.mtu)
        ], check=True)
        
        # Set bridge IP
        subprocess.run([
            "ip", "addr", "add", f"{self.gateway}/16", 
    ...
    def _configure_nat(self):
        ...
        check_cmd = [
            ...
            "-s", self.subnet, "!", "-o", self.bridge_name,
            "-j", "MASQUERADE"
        ]
    ...
    def connect_container(self, container_id: str, config: Dict[str, Any]) -> Dict[str, Any]:
...
class NetworkManager:
    ...
    def connect_container(self, container_id: str, network_name: str,
                         netns: str, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        result = self.cni_plugin.add_network(
            container_id, netns, ifname, net_config
        )
    ...
    def disconnect_container(self, container_id: str, network_name: str,
                           netns: str, ifname: str = "eth0"):
        ...
        self.cni_plugin.delete_network(
            container_id, netns, ifname, config
        )
    ...
    def list_networks(self) -> List[Dict[str, Any]]:
        ...
        for name, bridge in self.networks.items():
            ...
            networks.append({
                ...
                "containers": sum(1 for nets in self.container_networks.values() if name in nets),
                "ip_stats": stats
            })
    ...
    def _generate_subnet(self) -> str:
        ...
        used_subnets = {
            ipaddress.ip_network(bridge.subnet) 
            for bridge in self.networks.values()
        }
...
def main():
    ...
    nm.create_network(network_name, {
    ...
    try:
        ...
        for net in networks:
            ...
            print(f"  IPs allocated: {net['ip_stats']['allocated']}/{net['ip_stats']['total']}")
