    ...
    def save(self, git_dir: str):
        ...
        with open(os.path.join(rebase_dir, "git-rebase-todo"), "w") as f:
            for action in self.todo_list:
                f.write(
                    f"{action.command.value} {action.commit} {action.message or ''}\n"
                )
...
class RebaseOperation:
    ...
    def execute_rebase(self) -> RebaseResult:
        ...
        while self.state.todo_list:
            ...
            if result.success:
                ...
                # Error occurred
                return RebaseResult(
                    status="error", message=f"Failed to apply {action.commit[:7]}"
                )
    ...
    def continue_rebase(self) -> RebaseResult:
        ...
        if self.get_conflicted_files():
            return RebaseResult(status="error", message="Conflicts still present")
    ...
    def squash_with_previous(
        self, commit_sha: str, keep_message: bool = True
    ) -> ActionResult:
...
class BisectAlgorithm:
    ...
    def start(self, commit_graph: "CommitGraph"):
        ...
        if not commit_graph.is_ancestor(
            self.good_commits.pop(), self.bad_commits.pop()
        ):
    ...
    def calculate_commit_weights(self, commits: Set[str]) -> Dict[str, float]:
        ...
        for commit in commits:
            ...
            worst_case = max(
                total_untested - good_ancestors, total_untested - bad_descendants
            )
    ...
    def select_optimal_commit(self, weights: Dict[str, float]) -> str:
        ...
        merge_commits = [
            c for c in candidates if len(self.commit_graph.get_parents(c)) > 1
        ]
...
class CommitGraph:
    ...
    def get_commits_between(
        self, good_commits: Set[str], bad_commits: Set[str]
    ) -> Set[str]:
        """Get all commits between good and bad"""
...
def demo_rebase():
    ...
    rebase = RebaseOperation(
        onto="main", upstream="origin/main", branch="feature/my-feature"
    )
