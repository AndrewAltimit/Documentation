...
class RemoteProtocol:
    ...
    def push_pack(self, remote: Remote, 
        ...
        for refspec in refspecs:
            ...
            if not refspec.force and old_sha and not self._is_fast_forward(old_sha, new_sha):
                return PushResult(
                    False, 
                    {},
                    {refspec.destination: "Non-fast-forward update"}
                )
    ...
    def receive_pack(self, wants: List[str], 
                    common: Set[str]) -> bytes:
    ...
    def _send_pack_atomic(self, pack: bytes, 
                         updates: List[Tuple[str, Optional[str], str]]) -> PushResult:
    ...
    def _match_refs(self, patterns: List[str], remote_refs: Dict[str, str],
                   refspecs: List[str]) -> Dict[str, str]:
...
class DeltaCompression:
    ...
    def _find_match(self, data: bytes, hash_table: Dict[int, List[int]], 
                   source: bytes) -> Optional['Match']:
        ...
        for pos in hash_table[h]:
            ...
            match_len = 0
            while (match_len < len(data) and 
                   pos + match_len < len(source) and
                   data[match_len] == source[pos + match_len]):
                match_len += 1
    ...
    def _encode_delta(self, ops: List['DeltaOp'], 
                     source_size: int, target_size: int) -> bytes:
