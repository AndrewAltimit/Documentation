...
class ThreeWayMerge:
    ...
    def _compute_diff(self, from_lines: List[str], 
                     to_lines: List[str]) -> Dict[int, List[str]]:
...
class AdvancedMerge:
    ...
    @staticmethod
    def recursive_three_way_merge(base: List[str], ours: List[str], 
                                theirs: List[str]) -> MergeResult:
        """
    ...
    @staticmethod
    def patience_diff(a: List[str], b: List[str]) -> List[Tuple[str, int, int, int, int]]:
        """
        ...
        for idx_a, idx_b in matches:
            ...
            if idx_a > last_a or idx_b > last_b:
                ...
                for op, i1, i2, j1, j2 in section_diff:
                    diff_ops.append((op, last_a + i1, last_a + i2, 
                                   last_b + j1, last_b + j2))
    ...
    @staticmethod
    def _simple_diff(a: List[str], b: List[str]) -> List[Tuple[str, int, int, int, int]]:
    ...
    def semantic_merge(base: Dict, ours: Dict, theirs: Dict) -> Dict:
        ...
        for key in all_keys:
            ...
            if our_val == their_val:
                ...
                result[key] = our_val
            elif isinstance(base_val, dict) and isinstance(our_val, dict) and isinstance(their_val, dict):
