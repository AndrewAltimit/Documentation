...
```mermaid
flowchart LR
    Pod[Pod] -->|references| PVC[PersistentVolumeClaim<br/>'I need 10Gi fast SSD']
    PVC -->|selects| SC[StorageClass<br/>fast-ssd]
    SC -->|provisions via| CSI[CSI Driver]
    CSI -->|creates| PV[(PersistentVolume<br/>real disk)]
    PVC -.bound to.-> PV
    PV -.mounted into.-> Pod
```

With dynamic provisioning (the common case), you never create the PV by hand: the StorageClass and CSI driver create it on demand the moment a Pod claims the PVC.
...
<div class="notice--warning">
  ...
  <ul>
    <li><strong>No memory limit:</strong> A pod without a memory limit can consume the node and trigger system-level OOM kills that take neighbors down with it. Always set a memory limit.</li>
    <li><strong>CPU limits causing latency:</strong> Tight CPU limits throttle the container even when the node is idle. Set CPU <em>requests</em> for scheduling; be cautious with CPU <em>limits</em> on latency-sensitive services.</li>
    <li><strong>Treating Secrets as encrypted:</strong> Kubernetes Secrets are only base64-encoded at rest by default. Enable encryption at rest (or an external vault) and restrict access with RBAC.</li>
    <li><strong>ReadWriteMany on block storage:</strong> EBS/PD-backed volumes are ReadWriteOnce. RWX needs a file-based backend (EFS, NFS, CephFS) — a PVC requesting RWX against a block StorageClass stays Pending.</li>
    <li><strong>Default StorageClass reclaim policy:</strong> A <code>Delete</code> reclaim policy destroys the underlying volume when the PVC is deleted. Use <code>Retain</code> for data you cannot afford to lose.</li>
  </ul>
</div>
