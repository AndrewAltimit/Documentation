---
layout: docs
title: Andrew's Notebook
---

# Welcome to Andrew's Technical Documentation

A comprehensive knowledge base covering infrastructure, software development, physics, and artificial intelligence. This documentation serves as both a reference guide and learning resource for technical professionals and enthusiasts.

<div class="search-banner">
  <h2>🔍 <a href="search.html">Search Documentation</a></h2>
  <p>Quickly find what you're looking for across all documentation</p>
</div>

## Navigation Guides

- **[Getting Started](getting-started.html)** - New to this documentation? Start here
- **[Topic Map](docs/topic-map.html)** - Visual organization of all topics
- **[All Documentation](docs/index.html)** - Complete listing by category

## Technology

### Infrastructure & DevOps

- **[Terraform](docs/technology/terraform/)** - Infrastructure as Code
- **[Docker Essentials](docs/technology/docker-essentials.html)** - Container platform reference
- **[Docker](docs/technology/docker/)** - Containerization concepts and architecture
- **[Kubernetes](docs/technology/kubernetes/)** - Container orchestration
- **[AWS](docs/technology/aws/)** - Amazon Web Services cloud platform
- **[Networking](docs/technology/networking.html)** - Network fundamentals and protocols
- **[CI/CD](docs/technology/ci-cd.html)** - Continuous Integration and Deployment

### Development & Tools

- **[Git Command Reference](docs/technology/git-reference.html)** - Comprehensive Git commands and workflows
- **[Git](docs/technology/git.html)** - Version control system overview
- **[Branching Strategies](docs/technology/branching.html)** - Git workflow patterns
- **[Database Design](docs/technology/database-design.html)** - Database architecture and best practices
- **[Unreal Engine](docs/technology/unreal.html)** - Game engine development
- **[Please Build](docs/technology/please-build.html)** - Build system and automation

### Advanced Topics

- **[Artificial Intelligence](docs/technology/ai.html)** - AI/ML fundamentals and applications
- **[AI Fundamentals - Simplified](docs/technology/ai-fundamentals-simple.html)** - No-math introduction to AI
- **[AI Deep Dive](docs/technology/ai-lecture-2023.html)** - Advanced concepts and research
- **[Quantum Computing](docs/technology/quantumcomputing.html)** - Quantum algorithms and programming
- **[Cybersecurity](docs/technology/cybersecurity.html)** - Security principles and practices

## Physics

### Core Topics

- **[Classical Mechanics](docs/physics/classical-mechanics.html)** - Newtonian mechanics and dynamics
- **[Thermodynamics](docs/physics/thermodynamics.html)** - Heat, energy, and entropy
- **[Statistical Mechanics](docs/physics/statistical-mechanics.html)** - Microscopic basis of thermodynamics
- **[Relativity](docs/physics/relativity.html)** - Special and general relativity

### Advanced Physics

- **[Quantum Mechanics](docs/physics/quantum-mechanics.html)** - Quantum theory fundamentals
- **[Condensed Matter Physics](docs/physics/condensed-matter.html)** - Solid state physics
- **[Quantum Field Theory](docs/physics/quantum-field-theory.html)** - QFT principles and applications
- **[String Theory](docs/physics/string-theory.html)** - String theory and M-theory

## Specialized Hubs

### 🤖 [AI/ML Documentation Hub](docs/ai-ml/index.html)
Comprehensive guides for generative AI, including:
- [Stable Diffusion Fundamentals](docs/ai-ml/stable-diffusion-fundamentals.html)
- [ComfyUI Visual Programming](docs/ai-ml/comfyui-guide.html)
- [LoRA Training and Fine-tuning](docs/ai-ml/lora-training.html)
- [Model Architectures](docs/ai-ml/model-types.html) and [Base Models Comparison](docs/ai-ml/base-models-comparison.html)

### 📚 [Quick Reference Guide](docs/reference/index.html)
Essential references including:
- Command-line cheat sheets (Git, Docker, Kubernetes)
- API patterns and best practices
- Physics formulas and constants
- Algorithm complexity references

### 🔬 [Advanced Research Topics](docs/advanced/index.html)
Graduate-level content featuring:
- [AI Mathematics](docs/advanced/ai-mathematics/) - Statistical learning theory
- [Distributed Systems Theory](docs/advanced/distributed-systems-theory/) - Formal methods
- [Quantum Algorithms Research](docs/advanced/quantum-algorithms-research/) - Cutting-edge algorithms

### 🌐 [Distributed Systems Hub](docs/distributed-systems/index.html)
Architecture and implementation:
- [Kubernetes](docs/technology/kubernetes/) - Container orchestration
- [Docker](docs/technology/docker/) - Containerization fundamentals
- [Distributed Systems Theory](docs/advanced/distributed-systems-theory/)
- Microservices, consensus algorithms, and fault tolerance

## Recent Updates (2025)

- **Quantum Computing**: Added NISQ era algorithms and cloud platform guides
- **AI/ML**: Updated with Stable Diffusion 3 and FLUX documentation
- **Kubernetes**: Enhanced with v1.30 features and production patterns
- **Git**: Expanded with AI-assisted development workflows
- **Documentation**: Improved navigation and fixed broken links

---

*This documentation is continuously updated. For corrections or contributions, please visit our [GitHub repository](https://github.com/AndrewAltimit/Documentation).*