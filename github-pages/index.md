---
layout: docs
title: Andrew's Notebook
toc: false  # Index pages typically don't need TOC
---

# Andrew's Notebook

**[🔍 Search Documentation](search.html)**

## Table of Contents

**Technology**
- [Terraform](#terraform)
- [Docker](#docker)
- [AWS](#aws)
- [Kubernetes](#kubernetes)
- [Database Design](#database-design)
- [Networking](#networking)
- [Cybersecurity](#cybersecurity)
- [Git Version Control](#git-version-control)
- [Branching Strategies](#branching-strategies)
- [Unreal Engine](#unreal-engine)
- [Quantum Computing](#quantum-computing)
- [Artificial Intelligence](#artificial-intelligence)
- [Please Build](#please-build)

**Physics**
- [Classical Mechanics](#classical-mechanics)
- [Thermodynamics](#thermodynamics)
- [Statistical Mechanics](#statistical-mechanics)
- [Relativity](#relativity)
- [Quantum Mechanics](#quantum-mechanics)
- [Condensed Matter Physics](#condensed-matter-physics)
- [Quantum Field Theory](#quantum-field-theory)
- [String Theory](#string-theory)

## Technology

### Terraform
**[Andrew's Notes](docs/technology/terraform.html)**

Terraform is an infrastructure-as-code (IAC) tool that allows you to manage your infrastructure using a declarative language. Here are some useful resources and documentation for learning Terraform:

- [Official Terraform Documentation](https://www.terraform.io/docs/index.html)
- [Getting Started with Terraform](https://upcloud.com/resources/tutorials/get-started-terraform)
- [Terraform Best Practices](https://www.terraform-best-practices.com/)

### Docker
**[Andrew's Notes](docs/technology/docker.html)**

Docker is an open-source platform that automates the deployment, scaling, and management of applications using containers. Check out the following resources for more information on Docker:

- [Official Docker Documentation](https://docs.docker.com/)
- [Docker Getting Started Guide](https://docs.docker.com/get-started/)
- [Docker Compose Tutorial](https://docs.docker.com/compose/gettingstarted/)

### AWS
**[Andrew's Notes](docs/technology/aws.html)**

Amazon Web Services (AWS) is a comprehensive cloud computing platform offering a variety of services for computing, storage, databases, analytics, and more. Explore the following links to learn about AWS:

- [Official AWS Documentation](https://docs.aws.amazon.com/)
- [AWS Well-Architected Framework](https://aws.amazon.com/architecture/well-architected/)
- [AWS Certification Guide](https://aws.amazon.com/certification/)

### Unreal Engine
**[Andrew's Notes](docs/technology/unreal.html)**

Unreal Engine is a powerful game engine and development platform used for creating cutting-edge games and interactive experiences. Find resources on Unreal Engine below:

- [Official Unreal Engine Documentation](https://docs.unrealengine.com/)
- [Unreal Engine Beginner's Guide](https://docs.unrealengine.com/en-US/Basics/index.html)
- [Unreal Engine Marketplace](https://www.unrealengine.com/marketplace/)

### Quantum Computing
**[Andrew's Notes](docs/technology/quantumcomputing.html)**

Quantum computing is an emerging field that leverages the principles of quantum mechanics to process information. Dive into the world of quantum computing with these resources:

- [Introduction to Quantum Computing](https://qiskit.org/textbook/preface.html)
- [Quantum Computing Algorithms](https://quantum-computing.ibm.com/composer/docs/iqx/guide/)
- [Quantum Computing Hardware](https://www.ibm.com/quantum/systems)

### Artificial Intelligence
**[Andrew's Notes - Summary](docs/technology/ai.html)**

**[Andrew's Notes - Deep Dive](docs/technology/ai-lecture-2023.html)**

Artificial Intelligence (AI) is the development of computer systems that can perform tasks that would typically require human intelligence. Learn more about AI through the following links:

- [AI Foundations](https://www.deeplearning.ai/courses/)
- [Machine Learning Techniques](https://scikit-learn.org/stable/user_guide.html)
- [Deep Learning Frameworks](https://pytorch.org/tutorials/)

### Branching Strategies
**[Andrew's Notes](docs/technology/branching.html)**

Branching strategies are essential for managing the development process in a collaborative environment, ensuring that code changes are isolated, reviewed, and integrated systematically. Here are some popular branching strategies with placeholder links for further reading:

- [Git Branching Model](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/about-branches)
- [GitHub Flow](https://docs.github.com/en/get-started/quickstart/github-flow)
- [Git Feature Branch Workflow](https://www.atlassian.com/git/tutorials/comparing-workflows/feature-branch-workflow)

### Please Build
**[Andrew's Notes](docs/technology/please-build.html)**

Please is an open-source build automation tool that simplifies the process of building, testing, and packaging software. It offers an easy-to-understand syntax, high-performance execution, and supports a variety of languages and platforms.

- [Official Please Build Documentation](https://please.build/)
- [Getting Started with Please Build](https://please.build/quickstart.html)
- [Please Build Examples](https://github.com/thought-machine/please-examples)
- [Please Build vs. Other Build Tools](https://please.build/faq.html)

### Kubernetes
**[Andrew's Notes](docs/technology/kubernetes.html)**

Kubernetes is an open-source container orchestration platform that automates the deployment, scaling, and management of containerized applications. It provides a robust framework for running distributed systems resiliently.

- [Official Kubernetes Documentation](https://kubernetes.io/docs/)
- [Kubernetes The Hard Way](https://github.com/kelseyhightower/kubernetes-the-hard-way)
- [Kubernetes Best Practices](https://kubernetes.io/docs/concepts/configuration/overview/)
- [CNCF Kubernetes Certification](https://www.cncf.io/certification/cka/)

### Database Design
**[Andrew's Notes](docs/technology/database-design.html)**

Database design is the process of creating a detailed data model for a database, ensuring data integrity, optimizing performance, and meeting application requirements. It covers both relational and NoSQL database systems.

- [Database Design Tutorial](https://www.postgresql.org/docs/current/tutorial.html)
- [SQL vs NoSQL](https://www.mongodb.com/nosql-explained/nosql-vs-sql)
- [Database Normalization](https://www.geeksforgeeks.org/introduction-of-database-normalization/)
- [Performance Optimization](https://use-the-index-luke.com/)

### Networking
**[Andrew's Notes](docs/technology/networking.html)**

Computer networking is the practice of connecting computers and devices to share resources and communicate. Understanding networking fundamentals is essential for system administration, security, and modern application development.

- [OSI Model Explained](https://www.cloudflare.com/learning/ddos/glossary/open-systems-interconnection-model-osi/)
- [TCP/IP Protocol Suite](https://www.cisco.com/c/en/us/support/docs/ip/routing-information-protocol-rip/13769-5.html)
- [Network Security Basics](https://www.cisco.com/c/en/us/products/security/what-is-network-security.html)
- [Cloud Networking](https://aws.amazon.com/vpc/)

### Cybersecurity
**[Andrew's Notes](docs/technology/cybersecurity.html)**

Cybersecurity is the practice of protecting systems, networks, and data from digital attacks, unauthorized access, and damage. It encompasses technologies, processes, and practices designed to safeguard digital assets.

- [Security Fundamentals](https://www.nist.gov/cyberframework)
- [Cryptography Basics](https://cryptopals.com/)
- [Network Security](https://www.sans.org/cyber-security-courses/)
- [Security Best Practices](https://owasp.org/www-project-top-ten/)

### Git Version Control
**[Andrew's Notes](docs/technology/git.html)**

Git is a distributed version control system that enables developers to track changes in their code, collaborate with others, and maintain a complete history of their projects. It's the foundation of modern software development workflows.

- [Official Git Documentation](https://git-scm.com/doc)
- [Pro Git Book](https://git-scm.com/book/en/v2)
- [GitHub Learning Lab](https://lab.github.com/)
- [Atlassian Git Tutorial](https://www.atlassian.com/git/tutorials)

## Physics

### Classical Mechanics
**[Andrew's Notes](docs/physics/classical-mechanics.html)**

Classical Mechanics is the study of the motion of objects and the forces that cause this motion. It is based on Newton's laws of motion and covers a wide range of phenomena, from the motion of everyday objects to celestial bodies.

- [Introduction to Classical Mechanics](https://ocw.mit.edu/courses/8-01sc-classical-mechanics-fall-2016/)
- [Newton's Laws of Motion](https://www.khanacademy.org/science/physics/forces-newtons-laws)
- [Conservation Laws](https://www.feynmanlectures.caltech.edu/I_04.html)
- [Dynamics and Kinematics](https://www.physicsclassroom.com/class/1DKin)

### Thermodynamics
**[Andrew's Notes](docs/physics/thermodynamics.html)**

Thermodynamics is the branch of physics that deals with heat, work, temperature, and the statistical behavior of systems with large numbers of particles. It bridges the microscopic world with macroscopic properties.

- [Laws of Thermodynamics](https://chem.libretexts.org/Bookshelves/Physical_and_Theoretical_Chemistry_Textbook_Maps/Supplemental_Modules_(Physical_and_Theoretical_Chemistry)/Thermodynamics/The_Four_Laws_of_Thermodynamics)
- [Heat Engines and Refrigerators](https://www.khanacademy.org/science/physics/thermodynamics)
- [Phase Transitions](https://www.nature.com/subjects/phase-transitions-and-critical-phenomena)
- [Chemical Thermodynamics](https://chem.libretexts.org/Bookshelves/Physical_and_Theoretical_Chemistry_Textbook_Maps/DeVoes_Thermodynamics_and_Chemistry)

### Statistical Mechanics
**[Andrew's Notes](docs/physics/statistical-mechanics.html)**

Statistical mechanics provides the microscopic foundation for thermodynamics by connecting the behavior of individual particles to macroscopic observables through statistical methods.

- [Statistical Ensembles](https://ocw.mit.edu/courses/8-333-statistical-mechanics-i-statistical-mechanics-of-particles-fall-2013/)
- [Partition Functions](https://www.damtp.cam.ac.uk/user/tong/statphys.html)
- [Quantum Statistics](https://www.feynmanlectures.caltech.edu/III_04.html)
- [Phase Transitions and Critical Phenomena](https://arxiv.org/abs/0906.3454)

### Relativity
**[Andrew's Notes](docs/physics/relativity.html)**

Relativity is a branch of physics that deals with the relationship between space and time. It includes two major theories: Special Relativity and General Relativity, both formulated by Albert Einstein.

- [Special Relativity](https://www.einstein-online.info/en/elementary/)
- [General Relativity](https://arxiv.org/abs/gr-qc/9712019)
- [Spacetime](https://www.einstein-online.info/en/spotlights/)
- [Gravitational Waves](https://www.ligo.caltech.edu/page/what-are-gw)

### Quantum Mechanics
**[Andrew's Notes](docs/physics/quantum-mechanics.html)**

Quantum Mechanics is a fundamental theory in physics that describes the behavior of matter and energy at the atomic and subatomic scales. It departs from classical mechanics and introduces the concept of wave-particle duality, among other phenomena.

- [Introduction to Quantum Mechanics](https://ocw.mit.edu/courses/8-04-quantum-physics-i-spring-2016/)
- [Wave-Particle Duality](https://www.feynmanlectures.caltech.edu/III_01.html)
- [Heisenberg Uncertainty Principle](https://plato.stanford.edu/entries/qt-uncertainty/)
- [Quantum Tunneling](https://www.youtube.com/watch?v=WPZLRtyvEqo)

### Condensed Matter Physics
**[Andrew's Notes](docs/physics/condensed-matter.html)**

Condensed matter physics studies the physical properties of matter in its condensed phases, primarily solids and liquids. It encompasses phenomena from superconductivity to topological insulators.

- [Crystal Structure and Band Theory](https://www.doitpoms.ac.uk/tlplib/brillouin_zones/)
- [Superconductivity](https://www.aps.org/programs/outreach/history/historicsites/bednorz-mueller.cfm)
- [Magnetism and Magnetic Materials](https://magnetism.eu/esm/)
- [Topological Phases of Matter](https://arxiv.org/abs/1609.05953)

### Quantum Field Theory
**[Andrew's Notes](docs/physics/quantum-field-theory.html)**

Quantum Field Theory (QFT) is the theoretical framework that combines quantum mechanics with special relativity to describe the fundamental forces and particles of nature. It serves as the foundation for the Standard Model of particle physics.

- [Introduction to Quantum Field Theory](https://www.damtp.cam.ac.uk/user/tong/qft.html)
- [The Standard Model](https://home.cern/science/physics/standard-model)
- [Feynman Diagrams](https://www.quantumdiaries.org/2010/02/14/lets-draw-feynman-diagrams/)
- [Renormalization](https://arxiv.org/abs/hep-th/0212049)

### String Theory
**[Andrew's Notes](docs/physics/string-theory.html)**

String Theory is a theoretical framework that posits that the fundamental building blocks of the universe are one-dimensional "strings" rather than point particles. It aims to provide a unified description of gravity and particle physics, potentially reconciling general relativity and quantum mechanics.

- [Introduction to String Theory](https://www.damtp.cam.ac.uk/user/tong/string.html)
- [The Basics of Strings](https://arxiv.org/abs/hep-th/0207249)
- [M-Theory](https://arxiv.org/abs/hep-th/9607201)
- [Multiverse](https://arxiv.org/abs/hep-th/0610199)

