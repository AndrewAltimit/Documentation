...
class QuantumComputingAnimation(ThreeDScene):
    ...
    def classical_vs_quantum(self):
        ...
        qubit_eq = MathTex(
            r"|\\psi\\rangle = \\alpha|0\\rangle + \\beta|1\\rangle", font_size=36
        )
        qubit_eq.shift(RIGHT * 3)
        constraint = MathTex(
            r"|\\alpha|^2 + |\\beta|^2 = 1", font_size=28, color=YELLOW
        )
    ...
    def bloch_sphere_intro(self):
        ...
        state_vector = Arrow3D(
            start=[0, 0, 0], end=[1.4, 0, 1.4], color=YELLOW, thickness=0.05
        )
    ...
    def single_qubit_gates(self):
        ...
        for gate_name, gate_desc, gate_matrix, rotation in gates_data:
            ...
            if gate_name == "H":
                ...
                new_vector = Arrow3D(
                    start=[0, 0, 0], end=[2, 0, 0], color=YELLOW, thickness=0.05
                )
    ...
    def superposition_demo(self):
        ...
        self.play(
            ...
            bar_0_prob.animate.become(
                Text("100%", font_size=20, color=BLUE).move_to(bar_0)
            ),
            bar_1_prob.animate.become(
                Text("0%", font_size=20, color=RED).shift(RIGHT * 2 + DOWN * 0.5)
            ),
        )
    ...
    def entanglement_demo(self):
        ...
        # Create two qubits
        qubit1_label = Text("Qubit 1", font_size=24, color=BLUE).shift(
            LEFT * 4 + UP * 2
        )
        qubit2_label = Text("Qubit 2", font_size=24, color=RED).shift(
            RIGHT * 4 + UP * 2
        )
        ...
        self.play(
            Write(qubit1_label), Write(qubit2_label), Create(qubit1), Create(qubit2)
        )
        ...
        self.play(
            FadeOut(title),
            FadeOut(init_state),
            FadeOut(qubit1_label),
            FadeOut(qubit2_label),
            FadeOut(qubit1),
            FadeOut(qubit2),
            FadeOut(connection),
            FadeOut(cnot_text),
            FadeOut(entangled_text),
        )
    ...
    def deutschs_algorithm(self):
        ...
        # Hadamard gates
        h1_box = Square(side_length=0.5, color=BLUE).move_to(
            line1.get_start() + RIGHT * 2
        )
        h1_text = Text("H", font_size=20).move_to(h1_box)
        h2_box = Square(side_length=0.5, color=BLUE).move_to(
            line2.get_start() + RIGHT * 2
        )
        ...
        meter_needle = Line(
            meter.get_center(), meter.get_top(), color=RED, stroke_width=3
        )
    ...
    def quantum_advantage(self):
        ...
        # Exponential curve
        exp_curve = axes.plot(lambda x: 2**x, x_range=[0, 10], color=BLUE)
        ...
        # Future text
        future_text = Text("The Future of Computing", font_size=36, color=GREEN)
