...
class QuantumComputingDemo(ThreeDScene):
    def construct(self):
        ...
        subtitle = Text("Visualizing Qubits and Quantum Gates", font_size=30, color=GRAY)
    ...
    def classical_vs_quantum_simple(self):
        ...
        qubit_circle = Circle(radius=1.2, color=BLUE, fill_opacity=0.3).shift(RIGHT * 3 + DOWN)
        
        # Create superposition visual
        state_0 = Text("|0⟩", font_size=30, color=BLUE_C).shift(RIGHT * 3 + DOWN + UP * 0.5)
        state_1 = Text("|1⟩", font_size=30, color=RED_C).shift(RIGHT * 3 + DOWN + DOWN * 0.5)
        ...
        self.play(
            Write(alpha), Write(state_0),
            Write(plus_sign),
            Write(beta), Write(state_1)
        )
        ...
        self.play(
            *[FadeOut(obj) for obj in [
                section_title, bit_text, bit_0, bit_0_label, bit_1, bit_1_label,
                or_text, qubit_text, qubit_circle, state_0, state_1, plus_sign,
                alpha, beta, super_label
            ]]
        )
    ...
    def bloch_sphere_demo(self):
        ...
        # Clear labels
        self.play(
            FadeOut(title), FadeOut(h_gate_text), FadeOut(psi_label),
            FadeOut(plus_state), FadeOut(x_label), FadeOut(y_label),
            FadeOut(z_label), FadeOut(zero_state), FadeOut(one_state)
        )
    ...
    def entanglement_visual(self):
        ...
        self.play(
            ...
            Write(qubit1_state), Write(qubit2_state)
        )
        ...
        # Show correlation
        measure_text = Text("Measuring one instantly affects the other!", font_size=25, color=GREEN)
        ...
        self.play(
            *[FadeOut(obj) for obj in [
                title, qubit1, qubit2, qubit1_label, qubit2_label,
                qubit1_state, qubit2_state, state_text, operation_text,
                connection, entangled_label, measure_text
            ]]
        )
