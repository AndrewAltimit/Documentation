...
class QuantumDemoAdvanced(ThreeDScene):
    ...
    def measurement_visualization(self):
        ...
        new_bars = VGroup()
        new_bars.add(
            Rectangle(width=1, height=2.5, color=GRAY, fill_opacity=0.2).shift(LEFT * 3),
            Rectangle(width=0.8, height=0.1, color=BLUE, fill_opacity=0.8).shift(LEFT * 3 + DOWN * 1.2),
            Text("0%", font_size=16).shift(LEFT * 3 + UP * 1.5),
            Rectangle(width=1, height=2.5, color=GRAY, fill_opacity=0.2).shift(RIGHT * 3),
            Rectangle(width=0.8, height=2.4, color=RED, fill_opacity=0.8).shift(RIGHT * 3 + UP * 0.2),
    ...
    def quantum_supremacy(self):
        ...
        ).arrange(RIGHT, buff=0.5).shift(DOWN * 3)
