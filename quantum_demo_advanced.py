...
class QuantumDemoAdvanced(ThreeDScene):
    def construct(self):
        # Ultra-fast intro (0.3s)
        title = Text("QUANTUM COMPUTING", font_size=52, color=BLUE, weight=BOLD)
        self.add(title)
        self.play(title.animate.scale(0.1).set_opacity(0), run_time=0.3)

        # Core demonstrations with perfect timing
        self.superposition_3d()  # 3.5s
        self.measurement_visualization()  # 3s
        self.entanglement_spooky()  # 3.5s
        self.quantum_gates_circuit()  # 3s
        self.quantum_supremacy()  # 2s

        # Quick outro (0.5s)
        end = Text("QUANTUM", font_size=48, color=BLUE, weight=BOLD)
        end2 = Text("COMPUTING", font_size=48, color=PURPLE, weight=BOLD)
        end2.next_to(end, DOWN)
        self.play(Write(end), Write(end2), run_time=0.5)
        self.wait(0.2)
    ...
    def superposition_3d(self):
        ...
        self.play(Create(bit_state[0]), Write(bit_state[1]), run_time=0.3)

        # Quick flips to show discrete states
        for _ in range(3):
            self.play(
                bit_state[0].animate.set_fill(BLACK, 1),
                bit_state[1]
                .animate.set_color(WHITE)
                .become(Text("1", font_size=24, color=WHITE).shift(LEFT * 4)),
                run_time=0.15,
            )
            self.play(
                ...
                .become(Text("0", font_size=24, color=BLACK).shift(LEFT * 4)),
                run_time=0.15,
            )
        ...
        state_vector = Arrow3D(
            start=[3, 0, 0], end=[3, 0, 1.2], color=YELLOW, thickness=0.04
        )

        # Probability amplitudes (live updating)
        prob_display = VGroup()
        alpha_text = Text("α = 1.00", font_size=14, color=BLUE)
        beta_text = Text("β = 0.00", font_size=14, color=RED)
        alpha_text.shift(RIGHT * 3 + DOWN * 2.2)
        beta_text.shift(RIGHT * 3 + DOWN * 2.6)
        ...
        def update_state(mob, dt):
            # Rotate state vector
            mob.rotate(2 * dt, axis=np.array([1, 1, 0]), about_point=[3, 0, 0])
            ...
            alpha_text.become(
                Text(f"α = {alpha:.2f}", font_size=14, color=BLUE).shift(
                    RIGHT * 3 + DOWN * 2.2
                )
            )
            beta_text.become(
                Text(f"β = {beta:.2f}", font_size=14, color=RED).shift(
                    RIGHT * 3 + DOWN * 2.6
                )
            )
    ...
    def measurement_visualization(self):
        ...
        collapsed_vector = Arrow(
            ORIGIN, DOWN * 1.8, color=YELLOW, buff=0, stroke_width=6
        )

        # Update probabilities
        new_bars = VGroup()
        new_bars.add(
            Rectangle(width=1, height=2.5, color=GRAY, fill_opacity=0.2).shift(
                LEFT * 3
            ),
            Rectangle(width=0.8, height=0.1, color=BLUE, fill_opacity=0.8).shift(
                LEFT * 3 + DOWN * 1.2
            ),
            Text("0%", font_size=16).shift(LEFT * 3 + UP * 1.5),
            Rectangle(width=1, height=2.5, color=GRAY, fill_opacity=0.2).shift(
                RIGHT * 3
            ),
            Rectangle(width=0.8, height=2.4, color=RED, fill_opacity=0.8).shift(
                RIGHT * 3 + UP * 0.2
            ),
            Text("100%", font_size=16).shift(RIGHT * 3 + UP * 1.5),
        )
    ...
    def entanglement_spooky(self):
        ...
        self.play(FadeIn(source), run_time=0.2)
        ...
        self.play(Write(bell_state), run_time=0.3)

        # Quantum connection visualization
        connection = VGroup()
        for i in range(5):
            wave = ParametricFunction(
                lambda t: np.array([-3 + 6 * t, 0.2 * np.sin(10 * t), 0]),
                t_range=[0, 1],
                color=PURPLE,
                stroke_width=3,
            )
        ...
        self.play(Create(distance_arrow), Write(distance_text), run_time=0.3)
        ...
        self.play(Write(measure_a), Flash(particle_a[0], color=GREEN), run_time=0.3)

        # Alice collapses
        self.play(
            particle_a[1].animate.put_start_and_end_on(
                particle_a[0].get_center(), particle_a[0].get_center() + DOWN * 0.6
            ),
            run_time=0.3,
        )
        ...
        self.play(
            correlation_pulse.animate.move_to(particle_b[0].get_center())
            .scale(20)
            .set_opacity(0),
            run_time=0.4,
        )
        ...
        self.play(
            particle_b[1].animate.put_start_and_end_on(
                particle_b[0].get_center(), particle_b[0].get_center() + DOWN * 0.6
            ),
        ...
        self.play(Write(result_a), Write(result_b), run_time=0.2)

        # Einstein quote
        spooky_text = Text(
            '"Spooky Action at a Distance"', font_size=20, color=PURPLE, slant=ITALIC
        )
        einstein = Text("- Einstein", font_size=14, color=GRAY)
        spooky_text.shift(DOWN * 3)
        einstein.next_to(spooky_text, DOWN)

        self.play(Write(spooky_text), Write(einstein), run_time=0.5)
    ...
    def quantum_gates_circuit(self):
        ...
        self.play(Create(qubit_lines), Write(init_states), run_time=0.3)
        ...
        self.play(Create(h_gate), run_time=0.2)
        ...
        self.play(Create(h_effect), run_time=0.3)
        ...
        self.play(Create(cnot_gate), run_time=0.3)
        ...
        connection_effect = DashedLine(
            UP * 0, DOWN * 1.5, color=PURPLE, stroke_width=4
        ).shift(RIGHT * 1.5)

        self.play(Write(entangle_effect), Create(connection_effect), run_time=0.3)
        ...
        self.play(Create(measure_symbols), run_time=0.3)
    ...
    def quantum_supremacy(self):
        ...
        self.play(Create(axes), Write(x_label), Write(y_label), run_time=0.3)

        # Classical line (linear)
        classical_line = axes.plot(lambda x: 10 * x, x_range=[0, 10], color=RED)
        ...
        quantum_curve = axes.plot(lambda x: 2**x, x_range=[0, 10], color=BLUE)
        ...
        self.play(Create(points), run_time=0.3)

        # Real applications
        apps_title = Text("APPLICATIONS", font_size=20, color=GREEN)
        apps_title.shift(DOWN * 2.5)

        apps = (
            VGroup(
                Text("• Drug Discovery", font_size=16),
                Text("• Cryptography", font_size=16),
                Text("• AI/ML", font_size=16),
                Text("• Finance", font_size=16),
            )
            .arrange(RIGHT, buff=0.5)
            .shift(DOWN * 3)
        )

        self.play(Write(apps_title), Write(apps), run_time=0.5)
