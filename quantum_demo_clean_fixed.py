...
class QuantumDemoClean(Scene):
    ...
    def qubit_superposition(self):
        ...
        quantum_group.add(quantum_label, sphere, vector, prob_bg, 
                         prob_0_bar, prob_1_bar, prob_0_text, prob_1_text)
        ...
        def update_quantum_state(mob, alpha):
            angle = alpha * PI
            # Update vector
            new_end = sphere.get_center() + 1.2 * np.array([np.sin(angle), np.cos(angle), 0])
            ...
            prob_0_text.become(Text(f"|0⟩: {int(p0*100)}%", font_size=14).shift(RIGHT * 1.3 + DOWN * 2.2))
            
            prob_1_bar.set_width(max(0.01, p1 * 1.4))
            prob_1_bar.move_to(RIGHT * (2 + p1 * 0.7) + DOWN * 1.5)
            prob_1_text.become(Text(f"|1⟩: {int(p1*100)}%", font_size=14).shift(RIGHT * 2.7 + DOWN * 2.2))
        ...
        self.play(
            ...
            Flash(sphere, color=GREEN),
            run_time=0.5
        )
        
        self.wait(0.2)
        self.play(FadeOut(Group(classical_group, quantum_group, state_eq, superposition_text)), run_time=0.3)
    ...
    def measurement_collapse(self):
        ...
        # Collapse animation
        self.play(
            Flash(vector, color=RED, flash_radius=1),
            run_time=0.3
        )
        ...
        self.play(
            ...
            outcome_group[5].animate.become(Text("100%", font_size=12, color=RED).next_to(outcome_group[3], RIGHT)),
        ...
        self.wait(0.3)
        self.play(FadeOut(Group(sphere, x_axis, y_axis, zero, one, plus, vector, psi, 
                               eq, measure_box, measure_text, outcome_group, collapse_text)), run_time=0.3)
    ...
    def entanglement_demo(self):
        ...
        alice_v = Arrow(alice.get_center(), alice.get_center() + UP * 0.8, 
        ...
        bob_v = Arrow(bob.get_center(), bob.get_center() + UP * 0.8,
                     color=YELLOW, buff=0, stroke_width=3)
        ...
        self.play(
            Create(VGroup(h_gate, h_text, cnot)),
            run_time=0.5
        )
        ...
        connection = DashedLine(alice.get_center(), bob.get_center(),
                               color=PURPLE, stroke_width=6, dash_length=0.1)
        ...
        self.play(
            pulse.animate.scale(5).fade_out(),
            run_time=0.6
        )
        ...
        self.play(
            ...
            alice_v.animate.put_start_and_end_on(alice.get_center(), 
                                                alice.get_center() + DOWN * 0.8),
            run_time=0.4
        )
        ...
        self.play(
            bob_v.animate.put_start_and_end_on(bob.get_center(),
        ...
        # Spooky action
        spooky = Text("\"Spooky Action at a Distance\"", font_size=22, 
                     color=PURPLE, slant=ITALIC)
        spooky.shift(DOWN * 3)
        self.play(Write(spooky), run_time=0.5)
        
        self.wait(0.5)
        self.play(FadeOut(Group(alice, bob, alice_v, bob_v, alice_label, bob_label,
                               state, connection, alice_result, bob_result, measure_text, 
                               spooky, pulse)), run_time=0.3)
    ...
    def quantum_gates(self):
        ...
        # Show effect
        x_before = Arrow(LEFT * 4 + LEFT * 1.5, LEFT * 4 + LEFT * 1.5 + UP * 0.6,
                        color=YELLOW, buff=0)
        x_after = Arrow(LEFT * 4 + RIGHT * 1.5, LEFT * 4 + RIGHT * 1.5 + DOWN * 0.6,
                       color=YELLOW, buff=0)
        ...
        gates.add(x_gate, x_text, x_desc, x_before, x_after,
                 h_gate, h_text, h_desc, h_before, h_after,
                 z_gate, z_text, z_desc)
        ...
        self.wait(0.5)
        self.play(FadeOut(Group(gates, inter_title, path1, path2, result)), run_time=0.3)
    ...
    def quantum_advantage(self):
        ...
        # Visual comparison
        classical = Text("Classical: n bits", font_size=20, color=RED).shift(LEFT * 3 + UP * 2)
        quantum = Text("Quantum: n qubits", font_size=20, color=BLUE).shift(RIGHT * 3 + UP * 2)
        ...
        for i, (n, classical_states, quantum_states) in enumerate([
            ...
            example = VGroup(
                ...
                Text(f"{classical_states} state", font_size=16, color=RED).shift(LEFT * 2 + DOWN * (i * 0.8)),
                Text("vs", font_size=16).shift(DOWN * (i * 0.8)),
                Text(f"{quantum_states} states", font_size=16, color=BLUE).shift(RIGHT * 2.5 + DOWN * (i * 0.8))
            )
        ...
        # Applications
        apps = Text("Drug Discovery • Cryptography • AI • Optimization", 
