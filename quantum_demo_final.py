...
class QuantumDemoFinal(Scene):
    ...
    def qubit_vs_bit_visual(self):
        ...
        for _ in range(2):
            self.play(
                Transform(bit_0, bit_1),
                Transform(bit_0_text, bit_1_text),
                run_time=0.3
            )
            self.play(
                Transform(bit_0, Circle(radius=0.7, color=WHITE, fill_opacity=1).shift(LEFT * 4)),
                Transform(bit_0_text, Text("0", font_size=36, color=BLACK).move_to(bit_0)),
                run_time=0.3
            )
        ...
        # State equation
        state_eq = Text("α|0⟩ + β|1⟩", font_size=20, color=BLUE).shift(RIGHT * 3 + DOWN * 1.5)
        
        # Visual probability indicators
        prob_text = Text("Probabilities:", font_size=16).shift(RIGHT * 3 + DOWN * 2.2)
        prob_0 = Text("|α|² = 50%", font_size=14, color=BLUE).shift(RIGHT * 2 + DOWN * 2.7)
        prob_1 = Text("|β|² = 50%", font_size=14, color=RED).shift(RIGHT * 4 + DOWN * 2.7)
        ...
        for angle in np.linspace(0, 2*PI, 20):
            ...
            p1 = 100 - p0
            prob_0.become(Text(f"|α|² = {p0}%", font_size=14, color=BLUE).shift(RIGHT * 2 + DOWN * 2.7))
            prob_1.become(Text(f"|β|² = {p1}%", font_size=14, color=RED).shift(RIGHT * 4 + DOWN * 2.7))
    ...
    def superposition_measurement(self):
        ...
        outcome_0 = Circle(radius=0.3, color=BLUE, fill_opacity=0.8).shift(RIGHT * 4 + UP * 1.5)
        outcome_0_text = Text("0", font_size=16).move_to(outcome_0)
        outcome_0_prob = Text("50%", font_size=14).next_to(outcome_0, RIGHT)
        
        outcome_1 = Circle(radius=0.3, color=RED, fill_opacity=0.8).shift(RIGHT * 4 + DOWN * 1.5)
        outcome_1_text = Text("1", font_size=16).move_to(outcome_1)
        outcome_1_prob = Text("50%", font_size=14).next_to(outcome_1, RIGHT)
        
        self.play(
            Create(outcome_0), Write(outcome_0_text), Write(outcome_0_prob),
            Create(outcome_1), Write(outcome_1_text), Write(outcome_1_prob),
            run_time=0.5
        )
        ...
        self.play(
            ...
            super_eq.animate.become(Text("|ψ⟩ = |1⟩", font_size=22, color=RED).shift(DOWN * 3)),
            outcome_1.animate.scale(1.5),
            outcome_1_prob.animate.become(Text("100%", font_size=14, color=RED).next_to(outcome_1, RIGHT)),
    ...
    def entanglement_correlation(self):
        ...
        alice_vector = Arrow(alice_sphere.get_center(), alice_sphere.get_center() + UP,
                           color=YELLOW, buff=0, stroke_width=4)
        alice_label = Text("Alice", font_size=22, color=BLUE).next_to(alice_sphere, DOWN)
        
        bob_sphere = Circle(radius=1, color=RED, stroke_width=4).shift(RIGHT * 3.5)
        bob_vector = Arrow(bob_sphere.get_center(), bob_sphere.get_center() + UP,
                         color=YELLOW, buff=0, stroke_width=4)
        bob_label = Text("Bob", font_size=22, color=RED).next_to(bob_sphere, DOWN)
        
        self.play(
            Create(alice_sphere), Create(bob_sphere),
            Create(alice_vector), Create(bob_vector),
            Write(alice_label), Write(bob_label),
            run_time=0.5
        )
        ...
        self.play(
            pulse_ring.animate.scale(4).fade_out(),
            run_time=0.8
        )
        ...
        self.play(
            ...
            correlation_wave.animate.move_to(bob_sphere.get_center()).scale(15).fade_out(),
            run_time=0.6
        )
        ...
        # Spooky action
        spooky = Text("\"Spooky Action at a Distance\"", font_size=26, color=PURPLE, slant=ITALIC)
    ...
    def quantum_interference(self):
        ...
        slit2 = Rectangle(width=0.3, height=0.5, color=BLACK, fill_opacity=1).shift(DOWN)
        ...
        self.play(
            Create(source),
            Create(path1), Write(path1_label),
            Create(path2), Write(path2_label),
            run_time=0.8
        )
        ...
        # Key insight
        insight = Text("Quantum particles interfere with themselves!", 
    ...
    def quantum_advantage_simple(self):
        ...
        examples = VGroup(
            ...
            Text("50 qubits = 1,125,899,906,842,624 states!", font_size=18, color=YELLOW)
