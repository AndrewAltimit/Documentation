...
class QuantumDemoImproved(Scene):
    ...
    def qubit_vs_bit_enhanced(self):
        ...
        bit_group = VGroup()
        bit_0 = Circle(radius=0.6, color=WHITE, fill_opacity=0.8).shift(LEFT * 4 + UP * 0.5)
        bit_0_text = Text("0", font_size=30, color=BLACK).move_to(bit_0)
        bit_1 = Circle(radius=0.6, color=BLACK, fill_opacity=0.8).shift(LEFT * 4 + DOWN * 1.5)
        bit_1_text = Text("1", font_size=30, color=WHITE).move_to(bit_1)
        bit_label = Text("Classical Bit", font_size=20).shift(LEFT * 4 + DOWN * 2.5)
        
        # Show bit flipping
        self.play(
            Create(bit_0), Write(bit_0_text),
            Create(bit_1), Write(bit_1_text),
            Write(bit_label),
            run_time=0.8
        )
        ...
        # State vector
        vector = Arrow(sphere.get_center(), sphere.get_center() + UP * 1.2, 
        ...
        self.play(
            Create(sphere),
            Create(prob_0_bar), Create(prob_1_bar),
            Write(prob_0_label), Write(prob_1_label),
            Write(prob_0_value), Write(prob_1_value),
            Create(vector),
            Write(qubit_eq), Write(qubit_label),
            run_time=1
        )
        ...
        self.play(
            FadeOut(VGroup(
                title, bit_0, bit_0_text, bit_1, bit_1_text, bit_label,
                sphere, vector, prob_0_bar, prob_1_bar, prob_0_label, prob_1_label,
                prob_0_value, prob_1_value, qubit_eq, qubit_label, super_text
            )),
            run_time=0.5
        )
    ...
    def superposition_enhanced(self):
        ...
        self.play(
            Create(axes), Create(sphere),
            Write(zero), Write(one), Write(plus),
            run_time=0.8
        )
        
        # State vector with phase indicator
        vector = Arrow(LEFT * 3, LEFT * 3 + UP * 1.2, color=YELLOW, buff=0, stroke_width=5)
        phase_arc = Arc(radius=0.3, angle=0, color=GREEN).shift(LEFT * 3)
        
        # Probability distribution visualization
        prob_viz = VGroup()
        prob_title = Text("Measurement Probabilities", font_size=20).shift(RIGHT * 3 + UP * 2)
        
        # Create probability histogram
        prob_bars = VGroup()
        for i, (label, pos) in enumerate([("0", RIGHT * 2), ("1", RIGHT * 4)]):
            bar = Rectangle(width=0.8, height=0.01, color=BLUE if i == 0 else RED, 
                          fill_opacity=0.8).shift(pos)
            bar_label = Text(f"|{label}⟩", font_size=16).next_to(bar, DOWN)
            bar_value = DecimalNumber(0, num_decimal_places=1, font_size=14, 
                                    color=BLUE if i == 0 else RED)
            bar_value.next_to(bar, UP)
            prob_bars.add(VGroup(bar, bar_label, bar_value))
        
        self.play(
            Create(vector), Create(phase_arc),
            Write(prob_title), Create(prob_bars),
            run_time=0.8
        )
    ...
    def entanglement_enhanced(self):
        ...
        q1_vector = Arrow(q1_sphere.get_center(), q1_sphere.get_center() + UP * 0.8,
                         color=YELLOW, buff=0)
        ...
        q2_vector = Arrow(q2_sphere.get_center(), q2_sphere.get_center() + UP * 0.8,
                         color=YELLOW, buff=0)
        q2_label = Text("Bob", font_size=20).next_to(q2_sphere, DOWN)
        
        self.play(
            Create(q1_sphere), Create(q2_sphere),
            Create(q1_vector), Create(q2_vector),
            Write(q1_label), Write(q2_label),
            run_time=0.8
        )
        ...
        self.play(
            Create(h_gate), Write(h_label),
            Create(cnot_control), Create(cnot_target), Create(cnot_line),
            run_time=0.8
        )
        ...
        connection = DashedLine(
            q1_sphere.get_center(), q2_sphere.get_center(),
            color=PURPLE, stroke_width=4, dash_length=0.1
        )
        ...
        # Instant correlation
        correlation_text = Text("Bob's qubit instantly becomes |1⟩!", 
        ...
        self.play(
            ...
            correlation_wave.animate.move_to(q2_sphere.get_center()).scale(8).fade_out(),
            run_time=0.8
        )
        ...
        # Spooky action text
        spooky_text = Text("\"Spooky Action at a Distance\"", 
                          font_size=24, color=PURPLE, style=ITALIC)
    ...
    def quantum_gates_demo(self):
        ...
        path1 = CurvedArrow(
            LEFT * 3, RIGHT * 3, color=GREEN,
            angle=TAU/6
        ).shift(DOWN * 2.5)
        path2 = CurvedArrow(
            LEFT * 3, RIGHT * 3, color=RED,
            angle=-TAU/6
        ).shift(DOWN * 2.5)
        ...
        self.play(
            ...
            Write(path1_label), Write(path2_label),
            run_time=0.8
        )
        ...
        self.play(
            FadeIn(result_constructive), Write(const_label),
            FadeIn(result_destructive), Write(dest_label),
            run_time=0.8
        )
    ...
    def quantum_advantage(self):
        ...
        self.play(
            Create(quantum_curve), Write(quantum_label),
            Create(classical_curve), Write(classical_label),
            run_time=1.2
        )
