...
class QuantumDemo(Scene):
    ...
    def superposition_demo(self):
        ...
        state_label = Text("|ψ⟩", font_size=20, color=YELLOW).shift(
            UP * 2 + RIGHT * 0.3
        )
    ...
    def entanglement_demo(self):
        ...
        connection = Line(
            q1.get_center(), q2.get_center(), color=PURPLE, stroke_width=6
        )
        ...
        self.play(
            FadeOut(
                VGroup(
                    title, q1, q2, q1_label, q2_label, connection, bell_text, entangled
                )
            )
        )
