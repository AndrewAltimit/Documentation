...
class SimpleDemo(Scene):
    def construct(self):
        ...
        sine_curve = FunctionGraph(
            lambda x: np.sin(x), x_range=[-3, 3], color=RED
        ).shift(DOWN * 2)

        self.play(Create(sine_curve))

        # Oscillation
        self.play(circle.animate.shift(RIGHT * 6), rate_func=there_and_back, run_time=2)
        ...
        for _ in range(3):
            self.play(circle.animate.shift(RIGHT * 6), rate_func=sine_rate, run_time=2)
            self.play(circle.animate.shift(LEFT * 6), rate_func=sine_rate, run_time=2)
