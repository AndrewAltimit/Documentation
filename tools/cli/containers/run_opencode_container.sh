...
check_and_build_images() {
    ...
    if ! docker images | grep -q "Documentation-mcp-opencode"; then
        echo "📦 OpenCode MCP image not found, building..."
        images_missing=true
    fi

    # Check for mcp-crush image
    if ! docker images | grep -q "Documentation-mcp-crush"; then
        echo "📦 Crush MCP image not found, building..."
        images_missing=true
    fi

    # Check for openrouter-agents image
    if ! docker images | grep -q "Documentation-openrouter-agents"; then
    ...
    if [ "$images_missing" = true ]; then
        ...
        docker build -f docker/openrouter-agents.Dockerfile \
            --build-arg OPENCODE_IMAGE=Documentation-mcp-opencode:latest \
            --build-arg CRUSH_IMAGE=Documentation-mcp-crush:latest \
            -t Documentation-openrouter-agents:latest .
